/*
  ...
  1. Modifications
    - Renommage de la table affectation_personnel_entite en rh_affectation_personnel_entite
    - Mise à jour des contraintes et des index
    - Mise à jour des politiques RLS
    - Mise à jour des triggers

  2. Description
    Cette migration renomme la table affectation_personnel_entite en rh_affectation_personnel_entite
    pour maintenir une cohérence de nommage avec les autres tables du module RH.
*/

-- Vérification que la table existe avant de la renommer
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name = 'affectation_personnel_entite'
  ) THEN
