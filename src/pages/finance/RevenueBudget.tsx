...
function RevenueBudget() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Charger les budgets annuels
        const { data: annualData, error: annualError } = await supabase
          .from('v_budget_ca_mensuel')
          .select('entite_id, annee, type_service, total_annuel')
          .order('annee', { ascending: false });

        if (annualError) throw annualError;
        setAnnualBudgets(annualData || []);
  ...
  return (
    ...
    >
      <PageSection
        ...
        description="Vue synthétique des budgets par année"
      >
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2rem' }}>
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'right', padding: '8px', borderBottom: '2px solid #e5e7eb', fontSize: '0.875rem' }}>Total Annuel</th>
            </tr>
          </thead>
          <tbody>
            {annualBudgets.length === 0 ? (
              <tr>
                <td colSpan={4} style={{ padding: '8px', textAlign: 'center', fontSize: '0.875rem' }}>
                  Aucun budget annuel trouvé.
                </td>
              </tr>
            ) : annualBudgets.map((budget, index) => {
              const entite = entites.find(e => e.id === budget.entite_id);
              return (
                <tr key={`${budget.entite_id}-${budget.annee}-${budget.type_service}`}>
                  ...
                  <td style={{ padding: '8px', borderBottom: '1px solid #e5e7eb', fontSize: '0.875rem', textAlign: 'right' }}>
                    {budget.total_annuel.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                  </td>
                </tr>
              );
            })}
      ...
      <PageSection
        ...
        description="Détail des budgets mensuels par entité et type de service"
      >
